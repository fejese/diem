...
namespace Diem\FrontBundle\Routing;

use Diem\CoreBundle\Core\dmConfigurable;
use Symfony\Component\DependencyInjection\Container;
use Diem\CoreBundle\Entity\DmPage;
...
class dmPageRouting extends dmConfigurable
{
  ...
  public function __construct(Container $serviceContainer, array $options = array())
  {
  ...
  {
    $culture = null === $culture ? $this->serviceContainer->get('session')->get('user.culture', 'en') : $culture;
  ...
  protected function findPageForCulture($slug, $culture)
  {
    return $this->serviceContainer->get('doctrine.orm.entity_manager')
            ->getRepository('Diem\CoreBundle\Entity\DmPage')
            ->findOneBySlug($slug, $culture);
  }
